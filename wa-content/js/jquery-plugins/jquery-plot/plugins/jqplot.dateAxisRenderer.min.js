    ...
    $.jqplot.DateAxisRenderer.prototype.createTicks = function(plot) {
        ...
        // if user specified min and max are null, we set those to make best ticks.
            ...
            if (titarget <= 19*day) {
                ...
                min = new $.jsDate(min);
                var utc_offset = min.getUtcOffset();
                min = Math.floor((min.getTime() - utc_offset)/tempti) * tempti + utc_offset;

                max = new $.jsDate(max);
                max = Math.round((max.getTime() - max.getUtcOffset())/tempti) * tempti + utc_offset;

                nttarget = Math.ceil((max - min) / tempti) + 1;
                this.min = min;
                this.max = min + (nttarget - 1) * tempti;
                if(tempti == 86400000){
                    var tmp_t = new $.jsDate(this.min);
                    tmp_t = tmp_t.setDate(tmp_t.getDate()+(nttarget - 1));
                    this.max = tmp_t.getTime();
                }
                ...
                for (var i=0; i<nttarget; i++) {
                    ...
                    if(tempti == 86400000){
                        var tmp_t = new $.jsDate(this.min);
                        tmp_t = tmp_t.setDate(tmp_t.getDate()+i);
                        opts.value = tmp_t.getTime();
                    }
                    //alert(opts.value);
